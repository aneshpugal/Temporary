...
import log_sender.functions as func
...
def json_log_parser(lines_read):
    ...
    for event_obj in lines_read:
        try:
            ...
            if derived_eval:
                derived_fields(formatted_line,log_size)
...
def apply_masking(formatted_line,log_size):
...
def apply_hashing(formatted_line,log_size):
...
def derivedFields(formatted_line, log_size):
